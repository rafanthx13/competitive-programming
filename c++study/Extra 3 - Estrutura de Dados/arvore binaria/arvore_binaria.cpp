...
class Arvore {
  ...
  public:
    ...
    // inserir aux é recursiva, por isso está nessa estratégia top-down
    ...
    // ha várias ordens para imprimir
    // em ordem buscara na seguinte ordem 1. a esquerda,
    // 2. o elemento e so depois a direita
    void emOrdem(No* no){
      if(no != NULL){
        emOrdem(no->getEsq());
        cout << no->getChave() << " ";
...
int main(int argc, char *argv[]){
	...
	arv.emOrdem(arv.getRaiz()); // vai imprimir em ordem
